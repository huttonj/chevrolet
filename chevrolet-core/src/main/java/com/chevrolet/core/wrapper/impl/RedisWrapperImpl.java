package com.chevrolet.core.wrapper.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.chevrolet.core.enums.PrefixEnum;
import com.chevrolet.core.utils.KeyUtil;
...
public class RedisWrapperImpl implements RedisWrapper {
    ...
    @Override
    public <T> T getObjectComplex(String key, TypeReference typeReference) {
        String str = this.getString(key);
        if (null != str) {
            return (T) JSON.parseObject(str, typeReference);
        }
        return null;
    }
    ...
    @Override
    public void putAsync(String key, String json, int second) {
    ...
    @Override
    public Collection<String> getKeys(PrefixEnum prefixEnum) {
        Jedis jedis = getJedis();
        try {
            return jedis.keys(KeyUtil.getPatternKey(prefixEnum));
        } catch (Exception e) {
            log.error("redis error:", e);
            jedisPool.returnBrokenResource(jedis);
            jedis = null;
    ...
    @Override
    public Long addWithScore(String key, String value, long score) {
        Jedis jedis = getJedis();
        try {
    ...
    @Override
    public Long addWithScore(String key, String value, long score, int expireDays) {
        Jedis jedis = getJedis();
        try {
            Long l = jedis.zadd(key, score, value);
            expire(key, KeyUtil.getSecondByDays(expireDays));
            return l;
        } catch (Exception e) {
            log.error("redis error:", e);
            jedisPool.returnBrokenResource(jedis);
    ...
    public boolean lock(final String key, final int expire) {
        ...
        try {
            jedis = getJedis();
            String luaScript =
                    "local lockSuccess = redis.call('setnx',KEYS[1],ARGV[1]);" +
                    "local expireSuccess = 0;" +
                    "if lockSuccess == 1 then " +
                    "        expireSuccess = redis.call('expire',KEYS[1],ARGV[2]);" +
                    "        if expireSuccess == 0 then " +
            ...
            log.info("RedisWrapper lock e={}", e);
        } finally {
            close(jedis);
        }
        return false;
    ...
    @Override
    public boolean transactionActions(OptimisticLockCallback callback, String... keys){
        //传参keys用来做乐观锁监视用，如果callback中操作非keys内资源，不能保证线程安全
        Jedis jedis = null;
        try {
            jedis = getJedis();
            jedis.watch(keys);
            Transaction transaction = jedis.multi();
            callback.enqueue(transaction);
            ...
            log.error("RedisWrapper transactionActions e={}", e);
        } finally {
            jedis.unwatch();
            //归还jedis资源
            close(jedis);
